---
title: '8000020: экспорт контактов'
ms.author: daeite
author: daeite
manager: joallard
ms.date: 04/21/2020
ms.audience: Admin
ms.topic: article
ms.service: o365-administration
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.collection: Adm_O365
ms.custom:
- "8000020"
- "2003"
ms.assetid: ''
ms.openlocfilehash: d7dfdf30ec51f393cb47cb3fe89577923d4ac054579391f1fa9700bb9c3c6319
ms.sourcegitcommit: b5f7da89a650d2915dc652449623c78be6247175
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 08/05/2021
ms.locfileid: "53983286"
---
# <a name="export-contacts"></a>Экспорт контактов

1. В нижней части страницы в Outlook в Интернете выберите <img src='data:image/png;base64,' />
 **Люди**.
2. На панели инструментов выберите **Управление**  >  **экспортными контактами.** 
3. Выберите экспорт всех контактов или только контактов из определенной папки, а затем выберите **Экспорт**.
4. В нижней части страницы выберите **Сохранить,** чтобы сохранить "contacts.csv" в папке Загрузка по умолчанию. Или выберите стрелку рядом с **Сохранением и** выберите **Сохранить как**.

Дополнительные дополнительные новости см. [в Outlook.](https://support.office.com/article/10f09abd-643c-4495-bb80-543714eca73f#ID0EAACAAA=Outlook_on_the_web)

