---
title: Outlook.com списки контактов
ms.author: daeite
author: daeite
manager: joallard
ms.date: 04/21/2020
ms.audience: Admin
ms.topic: article
ms.service: o365-administration
ms.custom:
- "1850"
- "9000258"
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.openlocfilehash: a2bb861c4e8bc158b49138816553a37ff72ff8f0079cd6fdf98536697c857715
ms.sourcegitcommit: b5f7da89a650d2915dc652449623c78be6247175
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 08/05/2021
ms.locfileid: "54111953"
---
# <a name="organizing-your-outlookcom-mailbox"></a>Организация почтового ящика Outlook.com

Список контактов — это набор адресов электронной почты, который полезен для отправки электронной почты группе людей. Чтобы создать список контактов:

1. Перейти к [людям](https://outlook.live.com/people/) <img src='data:image/png;base64,' />.
1. В верхней части страницы выберите стрелку <img src='data:image/png;base64,' /> рядом с **Новым контактом,** а затем **выберите новый список контактов**.
1. Введите имя для списка, а затем добавьте имена или адреса электронной почты.
1. Нажмите **Создать**.

Дополнительные новости о создании, просмотре и редактировании контактов и списков контактов в [Outlook.com](https://support.office.com/article/5b909158-036e-4820-92f7-2a27f57b9f01?wt.mc_id=Office_Outlook_com_Alchemy).