---
title: '8000019: добавление или импорт контактов'
ms.author: daeite
author: daeite
manager: joallard
ms.date: 04/21/2020
ms.audience: Admin
ms.topic: article
ms.service: o365-administration
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.collection: Adm_O365
ms.custom:
- "8000019"
- "2002"
ms.assetid: ''
ms.openlocfilehash: 8e9535da046f21cc35ac9704492d8cd04e770aa997a6995f00dd3953241e471b
ms.sourcegitcommit: b5f7da89a650d2915dc652449623c78be6247175
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 08/05/2021
ms.locfileid: "53999793"
---
# <a name="import-contacts"></a>Импорт контактов

Перед импортом контактов в Outlook в Интернете необходимо экспортировать их из другого поставщика электронной почты в качестве .csv файла. Затем следуйте инструкциям ниже.

1. В нижней части страницы выберите <img src='data:image/png;base64,' />
 **Люди**.
2. На панели инструментов выберите **Управление**  >  **контактами импорта.**
3. Выберите **Просмотр,** выберите CSV-файл, а затем **откройте**.
4. Нажмите **Импорт**.

Дополнительные дополнительные новости см. в [Outlook.](https://support.office.com/article/bb796340-b58a-46c1-90c7-b549b8f3c5f8#ID0EAACAAA=Outlook_on_the_web)

